...
public class FatSecretAPI {
	...
	public String ProfileRequestScriptSessionKey(String token, String secret, int expires, int consumeWithin, String permittedReferrerRegex, boolean cookie) throws FatSecretException {
		...
		try{
			...
			oAuth.generateSignature(url, _consumerKey, _consumerSecret, token, secret, result);
	...
	private static class OAuthBase{
		...
		public void generateSignature(URL url, String consumerKey, String consumerSecret, String token, String tokenSecret, Result result) {
			GenerateSignatureBase(url, consumerKey, token, "POST", generateTimeStamp(), generateNonce(), "HMAC-SHA1", result);
